...
1.系统启动：  右键运行src/view/Start.java'
2.用户登录：  账号密码在config/user 中，格式为  用户名-密码-是否为管理员（1是0否）-权限
...
6.演示：①创建数据库111：create database 111
       ...
       ⑤新增数据：
			...
			Insert INTO stu1 (sname,class,grade) values ('张12','2022',85);                  //整形默认值0
			Insert INTO stu1 (sname,age,class) values ('张13',17,'2024');                     //double默认 0.0
			Insert INTO stu1 (sname,age,grade) values ('张14',18,65);                         //字符串默认“”
			Insert INTO stu1 (sname,age,class,grade) values ('张15',19,'2020',65);
       ⑥查询表：		
